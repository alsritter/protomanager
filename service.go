...
func New(cfg *Config, logger logger.Logger) (*Manager, error) {
	service := &Manager{
		cfg:     cfg,
		methods: &sync.Map{},
	...
	if cfg.Synchronization.Enable {
		synchronization, err := synchronization.New(cfg.Synchronization, logger)
		if err != nil {
			return nil, err
		}
...
func (s *Manager) startSynchronization(ctx context.Context, cancelFunc context.CancelFunc) error {
	...
	util.StartServiceAsync(ctx, s.Logger, cancelFunc, func() error {
